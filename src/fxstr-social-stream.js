(function( window ) {
	...
	.directive( 'socialStream', [ function() {
		...
		return {
			...
			, scope			: {
				...
				, socialStreamCachingTime		: '=' // Time in minutes that the social stream should be cached (localStorage)
			}
	...
	.controller( 'SocialStreamDirectiveController', [ '$scope', '$q', '$attrs', 'FacebookStreamService', 'TwitterStreamService', 'InstagramStreamService', 'SocialStreamStorageService', function( $scope, $q, $attrs, FacebookStreamService, TwitterStreamService, InstagramStreamService, SocialStreamStorageService ) {
		...
		$scope.$watchGroup( [ 'socialStreamFacebookPage', 'socialStreamFacebookProxy' ], function( newValue ) {
			...
			FacebookStreamService.getPosts( fbProxyUrl, fbPage )
				.then( function( fbPosts ) {
					$scope.status.facebook		= 2;
					$scope.posts				= $scope.posts.concat( fbPosts );
					SocialStreamStorageService.store( fbPosts, 'facebook', fbPage );
				}, function( err ) {
					console.error( err );
				} );
		...
		$scope.$watch( 'socialStreamTwitterId', function( newValue ) {
			...
			TwitterStreamService.getPosts( newValue )
				.then( function( twrPosts ) {
					$scope.status.twitter = 2;
					$scope.posts = $scope.posts.concat( twrPosts );
					SocialStreamStorageService.store( twrPosts, 'twitter', newValue );
				}, function( err ) {
					console.error( err );
				} );
		...
		$scope.$watchGroup( [ 'socialStreamInstagramToken', 'socialStreamInstagramUserId' ], function( newValue ) {
			...
			InstagramStreamService.getPosts( newValue[ 0 ], newValue[ 1 ] )
				.then( function( igrPosts ) {
					$scope.status.instagram = 2;
					$scope.posts = $scope.posts.concat( igrPosts );
					SocialStreamStorageService.store( igrPosts, 'instagram', newValue[ 1 ] );
				}, function( err ) {
					console.error( err );
				} );
	...
	.factory( 'SocialStreamStorageService', function() {

		return {
			store: function( data, serviceName, identifier ) {
			...
			/**
			* Retrieves social stream from localStorage
			* @param <Int> cachingTime		Time that the stream may be cached for
			*/
			, retrieve: function( serviceName, identifier, cachingTime ) {

				if( !window.localStorage || !cachingTime ) {
					return false;
				}
				...
				for( var i = 0; i < window.localStorage.length; i++ ) {
					...
					// Remove old posts
					if( keyName.substring( 0, 12 ) === 'socialStream' ) {
						var data = JSON.parse( window.localStorage.getItem( keyName ) );
						if( data.timeStamp < new Date().getTime() - cachingTime * 60 * 1000 ) {
							window.localStorage.removeItem( keyName );
						}
					}

					// Return entry if it's the right one
					if( keyName === 'socialStream-' + serviceName + '-' + identifier ) {
						var parsed = JSON.parse( window.localStorage.getItem( keyName ) );
